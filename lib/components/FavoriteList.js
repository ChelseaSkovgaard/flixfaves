...
class FavoriteList extends Component {
  ...
  componentDidMount() {
    ...
    let datePast = moment(date).subtract(28, 'days').calendar();
  ...
  findUpcomingSavedMovies() {
    let upcomingSavedMovies = filter(this.props.favorites, (movie) => {
      return movie.date > this.state.dateFuture
    });
    this.setState({upcomingSavedMovies: upcomingSavedMovies});
  }

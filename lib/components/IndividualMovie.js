...
export default class IndividualMovie extends Component {
  ...
  saveMovie(){
    ...
    console.log(this.props.movieDatabase)
  }
  ...
  render() {
    ...
    let image = this.props.movie.poster_path
    if(image === null) {
      image = "A1dBKcm1uBDqh2ovz2VRLe9Pktm.jpg"
    }

    return (
      <article className="individual-movie" id={(this.props.movie.id)}>

        <img onClick={() => {this.props.getMovieDetail(id)}} className="movie-image" src={"https://image.tmdb.org/t/p/w500/" + image }/>

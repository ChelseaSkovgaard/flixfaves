...
export default class IndividualMovie extends Component {
  ...
  saveMovie(){
    const newMovie = {
      ...
      date: moment(this.state.date).calendar(),
  ...
  render() {
    ...
    let date = moment(this.props.movie.release_date).calendar()
    return (
      <article className="individual-movie" id={(this.props.movie.id)}>
        <span><button className={classesFavorite}
        onClick={() => this.saveMovie()}></button></span>
